      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        >
          <div className="relative mb-4">
            <FaSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 text-neutral-100" />
            <input
              type="text"
              placeholder="search your resources...technology...."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-[#303030] pl-12 pr-4 rounded-sm py-2  text-white placeholder-gray-200 outline-none"
            />
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              className="flex items-center space-x-2 px-2 py-1 bg-[#262626] rounded-sm  text-md  transition-colors outline-none"
            >
              <FaFilter className="text-white" />
              <span className="text-white">Filters</span>
            </button>

            <div className="text-lg bg-[#303030] p-2  rounded-sm text-gray-100">
          ...
          <AnimatePresence>
            {showFilters && (
              <motion.div
                ...
                className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 p-4    rounded-sm"
              >
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#202020]   rounded-sm  px-1 py-2 text-white font-medium focus:outline-none"
                  >
          ...
          <AnimatePresence>
            {filteredAndSortedResources.map((resource, index) => (
              <motion.div
                ...
                className="  text-neutral-100 rounded-sm   p-4 outline-none transition-colors group"
              >
                {/* Header */}
                <div className="flex items-center justify-between mb-4">
                ...
                {/* Description */}
                <p className="text-gray-100 font-medium text-md mb-4 leading-relaxed">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {resource.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-1  text-sm text-blue-300"
                    >
                      #{tag}
                    </span>
                  ))}
                  ...
                  {resource.links.map((link, linkIndex) => (
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-white  p-2 rounded-sm font-bold underline   text-md">{link.title}</span>
                      </div>
                      <FaExternalLinkAlt className="text-white text-md opacity-0 group-hover/link:opacity-100 transition-opacity" />
                    </motion.a>
                  ))}
                </div>
              </motion.div>
        ...
        {filteredAndSortedResources.length === 0 && (
          ...
          >
            <div className="p-4 bg-gray-300 rounded-sm w-16 h-16 mx-auto mb-4 flex items-center justify-center">
